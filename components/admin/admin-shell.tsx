...
const navClass =
  "inline-flex min-h-9 items-center justify-center rounded-full px-4 py-2 text-sm font-semibold text-white transition-all duration-300 focus-ring hover:border-signal/30 hover:bg-signal/10 hover:text-signal";
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 py-8 md:px-8">
        <header className="sci-panel sci-border mb-8 flex flex-col gap-4 rounded-2xl border p-5 backdrop-blur-lg md:flex-row md:items-center md:justify-between">
          ...
          <nav className="flex flex-wrap items-center gap-2" aria-label="后台导航">
            ...
            <LogoutButton
              className={cn(
                "inline-flex min-h-9 items-center justify-center rounded-full border-2 border-white/20 bg-transparent px-4 py-2 text-sm font-semibold text-white transition-all duration-300 hover:border-signal/40 hover:bg-signal/10 focus-ring",
              )}
            />
          </nav>
        </header>
