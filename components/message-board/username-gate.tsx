...
export function UsernameGate({ onReady }: { onReady: (username: string) => void }) {
  ...
  if (username) {
    return (
      <GlassCard className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Badge live>Identity minted</Badge>
          <p className="mt-3 font-heading text-xl font-semibold text-white">{username}</p>
          <p className="text-sm text-stardust">你的留言身份已保存在本地浏览器。</p>
        </div>
  ...
  return (
    <GlassCard className="relative overflow-hidden p-6">
      <Sparkles className="absolute -right-6 -top-6 h-28 w-28 rotate-12 text-bitcoin/10" aria-hidden />
      <div className="mb-5 flex items-center gap-3">
        <div className="rounded-lg border border-bitcoin/50 bg-bitcoin/20 p-3 text-bitcoin shadow-[0_0_20px_rgba(234,88,12,0.25)]">
          <KeyRound className="h-5 w-5" aria-hidden />
        </div>
        <div>
          <Badge>Step 01</Badge>
          <h2 className="mt-2 font-heading text-2xl font-semibold text-white">创建留言身份</h2>
        </div>
      </div>
      <form onSubmit={saveUsername} className="space-y-4">
        <Input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="输入你的用户名，例如 Satoshi" aria-label="用户名" />
