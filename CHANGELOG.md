...
## 2026-06-24 · 长数字内容换行修复

### 说明

修复留言内容为长串数字或连续字符时不换行导致的样式溢出问题。

---

### 问题

当留言内容是长串连续数字（如 `54234234234234234242424242424323424324442423333333333333333`）或其他无空格的长字符串时，浏览器默认不会换行，导致：
- 卡片被内容撑开，破坏瀑布流布局
- 内容超出容器边界
- 整体页面样式错乱

**原因**：`break-words` 只在单词边界（空格、标点）处换行，对连续字符无效。

---

### 解决方案

将留言内容的 CSS 类从 `break-words` 改为 `break-all`：
- `break-all` 强制在任意字符处换行
- 保留 `whitespace-pre-wrap` 以支持用户输入的换行符
- 确保任何长内容都能正确折行，不会溢出容器

涉及文件：
- [components/message-board/message-list.tsx:116](components/message-board/message-list.tsx#L116) — 卡片列表中的留言预览
- [components/message-board/message-list.tsx:178](components/message-board/message-list.tsx#L178) — Modal 弹窗中的完整留言
